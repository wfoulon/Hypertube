...
class EpisodeComment extends Component {
  constructor(props) {
    super(props)
    this.state = {
      values: {},
  ...
  handleClick = (e) => {
    ...
    let seasonID = this.props.numSeason
    let { userName } = this.state.values
    let image = this.state.values.userImage
    Meteor.call('Send.comment_episode_serie', { comment, userId, id, tvdb, image, userName, seasonID })
...
export default withTracker(props => {
  ...
  let seasonID = props.numSeason
  return {
    CommentReady: Meteor.subscribe('Get_all_Serie_Episode_Comments', { id, tvdb, seasonID }).ready(),
    Comments: CommentDB.find({ id, tvdb, seasonID }, { sort: { 'date': -1 } }).fetch() || false
  }
})(EpisodeComment)
