import React, { Component } from 'react'
import { Input, Button, Mask } from 'mdbreact'
import './main.css'
import ReactPlayer from 'react-player'
import MovieComment from '../Comments/MovieComment.jsx'
import Slider from "react-slick"
import "slick-carousel/slick/slick.css"
import "slick-carousel/slick/slick-theme.css"
let imdb = require('imdb-api')
let userImage = 'data:image/png;base64,'

class MovieInfos extends Component {
  constructor(props) {
    super(props)
    this.state = {
      hover: false,
      data: '',
      dataImdb: '',
      allActors: null,
      director: null,
      prod: null,
      currPath: null
    }
    this.Mounted = false
    this.flag = true
    this.slider = null
    this.slider2 = null
    this.slider3 = null
  }

  onMouseEnterHandler = (e) => {
    if (this.Mounted) {
      this.setState({
        hover: true
      })
    }
  }
  onMouseLeaveHandler = (e) => {
    if (this.Mounted) {
      this.setState({
        hover: false
      })
    }
  }

  componentDidUpdate = (prevProps, prevState) => {
    if (prevProps.location.pathname.split('/')[2] !== this.props.location.pathname.split('/')[2]) {
      this.flag = true
      this.slider = null
      this.slider2 = null
      this.slider3 = null
      this.componentDidMount()
    }
  };
  

  componentWillUnmount = () => {
    this.Mounted = false
  }

  componentDidMount = async (e) => {
    this.Mounted = true
    let self = this
    var idMovie = window.location.pathname.split('/')[2]
    var params = {
      movie_id: idMovie
    }
    if (this.flag) {
      this.flag = false
      await Meteor.call('get_yts_details', { params }, (err, result) => {
        if (result) {
          let id = result.data.movie.imdb_code
          let infosImdb = new Promise((resolve, reject) => {
            resolve(this.funcImdb(id))
          })
          var genres = ''
          result.data.movie.genres.forEach(element => {
            genres = genres + ' / ' + element
          })
          result.data.movie.genres = genres.substring(3)
          result.data.movie.yt_trailer = "https://www.youtube.com/watch?v=" + result.data.movie.yt_trailer_code
          if (this.Mounted && infosImdb) {
            self.setState({
              dataImdb: infosImdb,
              data: result.data.movie,
              currPath: this.props.history.location.pathname
            })
          }
        }
      })
    }
  }

  funcImdb = async (id) => {
    let self = this
    Meteor.call('get_imdb_infos', { id }, (err, res) => {
      if (res) {
        if (this.Mounted) {
          if (!self.state.allActors)
            self.renderActors(res)
          if (!self.state.prod)
            self.renderProd(res)
        }
        if (res) return (res)
      } else this.funcImdb(id)
    })
  }

  renderActors = async (data) => {
    if (data) {
      let imdbid = data.imdbid
      const all = await new Promise((resolve, reject) => {
        Meteor.call('get_tmdb_credits', { imdbid }, (err, result) => {
          if (result) resolve(result)
        })
      })
      let director = all.crew.map((data, key) => (
        <div key={key}>
          {data.job === 'Director' ?
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              <div >
                {data.profile_path === null ?
                  <img src={userImage} id='imgmaggle'></img> :
                  <img src={`https://image.tmdb.org/t/p/original${data.profile_path}`} id='imgmaggle'></img>}
              </div>
              <p className='font-weight-bold white-text' style={{ fontSize: '13px', marginRight: '2px', width: '100px', alignContent: 'center', textAlign: 'center', marginTop: 'auto' }} >{data.name}</p>
            </div> : ''
          }
        </div>
      ))
      if (this.Mounted) this.setState({ allActors: all.cast, director: director })
    }
  }

  renderProd = async (data) => {
    if (data) {
      let imdbid = data.imdbid
      const all = await new Promise((resolve, reject) => {
        Meteor.call('get_tmdb_infos', { imdbid }, (err, result) => {
          if (result) resolve(result)
        })
      })
      let prod = all.production_companies.map((data, key) => (
        <div key={key} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <div>
            {data.logo_path === null ?
              <i className="fas fa-question fa-3x"></i> :
              <img src={`https://image.tmdb.org/t/p/original${data.logo_path}`} id='imgmaggle' style={{ width: '80px', height: 'auto', borderRadius: '0%' }}></img>}
          </div>
          <p className='font-weight-bold white-text' style={{ fontSize: '13px', marginRight: '2px', width: '100px', alignContent: 'center', textAlign: 'center', marginTop: 'auto' }} >{data.name}</p>
        </div>
      ))
      if (this.Mounted && prod.length > 0) this.setState({ prod: prod })
    }
  }

  playMovie = (e, data) => {
    e.preventDefault()
    e.stopPropagation()
    this.props.display(e, data, 'movie')
  }

  render() {
    const { hover, data, dataImdb } = this.state
    let infos = false
    if (data && dataImdb) infos = true
    const code = data.yt_trailer_code
    return (
      <div style={{paddingBottom: '5rem'}}>
        {infos ?
          <div style={{ display: 'flex', flexDirection: 'column', marginLeft: '20px', marginTop: '20px' }}>
            <div className='divFlex'>
              <div>
                <div onMouseEnter={this.onMouseEnterHandler} onMouseLeave={this.onMouseLeaveHandler}>
                  {!hover ? <img src={data.large_cover_image} className='img-fluid imgDetails' alt='' style={{ minWidth: '350px' }} /> : ''}
                  {hover ?
                    <Mask overlay="stylish-strong" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', cursor: 'pointer' }} title='View details'>
                      <img src={data.large_cover_image} className='img-fluid' alt='' style={{ minWidth: '350px', backgroundColor: 'gray', opacity: '0.5' }} />
                      <i className="far fa-play-circle fa-7x cardPlayButton" onClick={e => this.playMovie(e, data)} style={{ position: 'absolute'}} title='View movie'></i>
                    </Mask>
                    : ''
                  }
                </div>
              </div>
              <div className='divCol'>
                <p className='font-weight-bold white-text' style={{ fontSize: '40px' }}>{data.title_english}</p>
                <p className='font-weight-bold white-text' style={{ fontSize: '20px' }}>{data.year}</p>
                <p className='font-weight-bold white-text' style={{ fontSize: '20px' }}>{data.genres}</p>
                <p className="font-weight-bold yellow-text" style={{ fontSize: '20px' }}>{data.rating}/10 ★</p>
                <p className='font-weight-bold white-text' style={{ fontSize: '16px', marginBottom: '-5px' }}>Summary</p>
                <p className='font-weight white-text' style={{ fontSize: '14px', marginRight: '10px' }}>{data.description_full}</p>
                <p className='font-weight-bold white-text' style={{ fontSize: '16px' }}>{dataImdb.runtime}</p>
                {this.state.director && this.state.director.length > 0 ?
                  <div style={{maxWidth: '100%', overflowX: 'scroll'}}>
                    <p className='font-weight-bold white-text' style={{ fontSize: '16px', marginBottom: '2px' }}>Director</p>
                    <div className='divFlex2'>
                      {this.state.director}
                    </div>
                  </div>
                  : ''}
                {this.state.allActors ?
                  <div style={{maxWidth: '100%', overflowX: 'scroll'}}>
                    <p className='font-weight-bold white-text' style={{ fontSize: '16px', marginBottom: '2px' }}>Cast</p>
                    <div className='divFlex2'>
                      {/* {this.state.allActors} */}
                      {this.state.allActors.map((data, key) => (
                        <div key={key} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                          <div >
                            {data.profile_path === null ?
                              <img src={userImage} id='imgmaggle'></img> :
                              <img src={`https://image.tmdb.org/t/p/original${data.profile_path}`} id='imgmaggle'></img>}
                          </div>
                          <p className='font-weight-bold white-text' style={{ fontSize: '13px', marginRight: '2px', width: '100px', alignContent: 'center', textAlign: 'center', marginTop: 'auto' }} >{data.name}</p>
                        </div>))}
                    </div>
                  </div>
                  : ''}
                {this.state.prod ?
                  <div style={{maxWidth: '100%', overflowX: 'scroll'}}>
                    <p className='font-weight-bold white-text' style={{ fontSize: '16px', marginBottom: '2px' }}>Productors</p>
                    <div className='divFlex2'>
                      {this.state.prod}
                    </div>
                  </div>
                  : ''}
              </div>
            </div>
            {data.yt_trailer && data.yt_trailer !== 'https://www.youtube.com/watch?v=' ?
              <div style={{ marginTop: '20px', display: 'flex', justifyContent: 'center'}}>
                <iframe width="560" height="315" src={`https://www.youtube.com/embed/${code}`} frameBorder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowFullScreen></iframe>
                {/* <ReactPlayer url={data.yt_trailer} controls /> */}
              </div>
              : ''}
          </div>
          : ''}
        {!!this.state.data &&
          <MovieComment browserHistory={this.props.history} data={this.state.data}></MovieComment>
        }
      </div>
    )
  }
}

export default MovieInfos
